...
if ((Get-ExecutionPolicy) -eq 'Restricted') {
    Write-Host "Automatically setting execution policy to RemoteSigned..."
...
if ((Test-Path "$DriveLetter\sources\boot.wim") -eq $false -or (Test-Path "$DriveLetter\sources\install.wim") -eq $false) {
    if ((Test-Path "$DriveLetter\sources\install.esd") -eq $true) {
        ...
        Write-Host "Using image index: $InstallIndex (from parameter)"
        Write-Host 'Converting install.esd to install.wim. This may take a while...'
        & 'DISM' /Export-Image /SourceImageFile:"$DriveLetter\sources\install.esd" /SourceIndex:$InstallIndex /DestinationImageFile:"$mainOSDrive\nano11\sources\install.wim" /Compress:max /CheckIntegrity
    } else {
...
& dism /English "/mount-image" "/imagefile:$($env:SystemDrive)\nano11\sources\install.wim" "/index:$InstallIndex" "/mountdir:$($env:SystemDrive)\scratchdir"
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$wimFilePath" "/index:$InstallIndex"
...
# Removed: Read-Host "Press Enter to perform cleanup and exit."
